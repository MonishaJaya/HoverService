package com.robotic.demo.service;
...
import com.robotic.demo.validator.Validator;
 ...
 * 
 * @param hooverRequest the request containing room size, initial coordinates, patches, and instructions
 * @return ResponseEntity containing the final coordinates and the number of cleaned patches
 */
    public ResponseEntity<HooverResponse> cleanService(HooverRequest hooverRequest) {
        ...
        if (org.apache.commons.lang3.StringUtils.isNotBlank(hooverRequest.getInstructions())) {
        	...
        	//If no instruction , check if the initial coordinates match any patch and clean it
        	if(patches.stream().anyMatch(patch -> Arrays.equals(patch, currentCoordinates))) {
        		patchesCleaned.getAndIncrement();
        	}
        }
