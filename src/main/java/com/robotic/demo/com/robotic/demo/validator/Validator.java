package com.robotic.demo.validator;
...
public class Validator {
	/**
	 * Validates the room size coordinates , patches, and instructions in the HooverRequest.
	 ...
	 public void validateRoomSize(HooverRequest hooverRequest) {
	    ...
	    public void isValidCoordinates(int[] coords, int[] roomSize) {
	    	if (coords[0] < 0 || coords[0] >= roomSize[0] || coords[1] < 0 || coords[1] >= roomSize[1]) { 
	    		throw new ValidationException("Invalid coordinates, the hoover is out of the room probably given invalid instructions.");  
	    } }
}
